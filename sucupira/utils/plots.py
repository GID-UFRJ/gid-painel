...
class PlotsPessoal:
    ...
    def _entidades_por_ano(
        ...
        agregacao: str | object = "count",  # count | sum | avg | max | min ou instância
        distinct: bool = True,
    ):
        """
        Função genérica para gerar gráficos por ano.
        - Suporta qualquer modelo.
        - Suporta qualquer agrupamento (ou nenhum).
        - Suporta agregações Count, Sum, Avg, Max, Min.
        """
        ...
        if isinstance(agregacao, str):
            ...
            # caso o usuário passe Count, Sum, etc. diretamente
            agg_func = agregacao
        ...
        if campo_grupo:
            dados = (
                queryset
                .values("ano__ano_valor", campo_grupo)
                .annotate(total=agg_func)
