...
agrupamentos_disponiveis_individuos = {
            'Sexo': 'sexo',
...
# =============================================================================
# TAG PARA A SEÇÃO "POSGRAD_UFRJ" (todas as PPGs da UFRJ)
# =============================================================================

@register.inclusion_tag("common/partials/_layout_de_filtros_dinamico.html", takes_context=True)
def render_filtros_posgrad_ufrj(context, tipo, url_grafico, grafico_id, spinner_id, grafico_html, plotter_name, nome_plot):
    """
    Monta o formulário de filtros para a seção "Pessoal" compondo filtros atômicos.
    """
    ctx = { "url_grafico": url_grafico, 
    ...
    if tipo == "programas_contagem":
        ...
        ctx.update({
            ...
            "modalidades": ProgramaModalidade.objects.all()
        })
        agrupamentos_disponiveis = {
            'Conceito CAPES': 'conceito',
            'PPG em Rede': 'in_rede',
            # Adicione aqui se tiver mais agrupamentos para este tipo
        }

    else:
        # Adicione outros 'elif' para outros tipos de gráfico nesta seção, se houver
        raise ValueError(f"Tipo de filtro inválido para 'posgrad_ufrj': {tipo}")
